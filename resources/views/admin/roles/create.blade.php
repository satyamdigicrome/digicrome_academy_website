...
<style>
  ...
  .form-page .section-label { font-size:.75rem; font-weight:700; text-transform:uppercase; letter-spacing:.5px; color:#1a237e; padding-bottom:8px; border-bottom:2px solid #e8eaf6; margin-bottom:16px; }
  ...
  <form action="{{ route('admin.roles.store') }}" method="POST">
    ...
    <div class="row g-4">
          ...
          <div class="form-card-body">
            <div class="mb-3">
              ...
              <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                value="{{ old('name') }}" placeholder="e.g. Content Editor" required>
              @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="mb-3">
              <label class="form-label">Description</label>
              <textarea name="description" class="form-control" rows="2"
                placeholder="What does this role do?">{{ old('description') }}</textarea>
            </div>

            <label class="toggle-wrap" id="superAdminToggle">
              ...
              <div>
                ...
                <div style="font-size:.75rem;color:#888">Grants unrestricted access to ALL modules (no need to select permissions)</div>
              </div>
            </label>
            <div class="super-notice" id="superNotice">
              ℹ️ Super Admin roles have access to all modules automatically. The permission checkboxes below are ignored.
            </div>
          </div>
        </div>
        ...
        <div class="form-card" id="permissionsCard">
          ...
          <div class="form-card-body">
            <p style="font-size:.85rem;color:#666;margin-bottom:16px;">
              Select which CMS modules this role can access. Users with this role will only see and interact with the checked modules.
            </p>
            ...
            <div class="perm-grid" id="permGrid">
              ...
              <label class="perm-item {{ in_array($perm->id, old('permissions', [])) ? 'checked' : '' }}">
      ...
      <div class="col-12 d-flex gap-3 pt-2">
        <button type="submit" class="btn-submit"><i class="bi bi-plus-circle-fill me-2"></i>Create Role</button>
